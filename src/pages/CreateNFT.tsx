...
const CreateNFT: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Check wallet balance and program state
      ...
      try {
        ...
        console.log('Program state found, certificate count:', state.certificateCount.toString())
      ...
      // Use ipfs:// format for blockchain storage
      const metadataUri = pinataService.getIPFSUrl(metadataUploadResponse.IpfsHash)
      console.log('Metadata URI for blockchain:', metadataUri)
      ...
      const nftData = {
        ...
        imagePreview: preview // Include the preview image (can be null)
      }

      // Add NFT to context (this will save to localStorage and update collection)
      addNFT(nftData, nftResult)
      updateStep(7, 'completed', 'NFT added to your collection successfully')

      // Set result for display (use gateway URL for viewing)
      setNftResult({
        ...nftResult,
        metadataUri: pinataService.getGatewayUrl(metadataUploadResponse.IpfsHash)
      })
      ...
      // Show success notification and auto-redirect after 3 seconds
      setTimeout(() => {
        navigate('/collection')
      }, 3000)
  ...
  return (
    <div className="create-nft-page">
      <div className="container">
        ...
        {/* Show creation progress if uploading */}
        {isUploading && (
